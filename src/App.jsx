...
export default function App() {
  ...
  // BLOQUEIO TOTAL: Se não autenticado, força estritamente a tela de LOGIN
  if (!isAuthenticated) {
    return (
  ...
  // PAINEL LOGADO (Acessível somente após login)
  return (
    <div className="bg-gray-100 font-sans min-h-screen flex flex-col items-center justify-start p-4">
      ...
      <header className="no-print w-full max-w-5xl flex justify-center gap-4 mb-6">
        <button
          onClick={() => setVerCertificado(false)}
          className="bg-slate-800 hover:bg-slate-900 text-white font-medium px-4 py-2 rounded shadow"
        >
          ⚙️ Painel Admin
        </button>
        {verCertificado && (
      ...
      <main className="w-full max-w-5xl">
        {!verCertificado ? (
          <div className="bg-white p-6 rounded shadow border">
            <h2 className="text-xl font-bold text-slate-800 mb-4">Painel de Gestão Escolar - MAZZ</h2>

            <div className="flex gap-2 mb-6 border-b pb-2">
              <button onClick={() => setActiveTab('alunos')} className={`px-3 py-1.5 text-xs font-bold rounded ${activeTab === 'alunos' ? 'bg-slate-800 text-white' : 'bg-gray-100'}`}>👨‍🎓 Cadastrar Alunos</button>
              <button onClick={() => setActiveTab('cursos')} className={`px-3 py-1.5 text-xs font-bold rounded ${activeTab === 'cursos' ? 'bg-slate-800 text-white' : 'bg-gray-100'}`}>📚 Cadastrar Cursos</button>
              <button onClick={() => setActiveTab('matriculas')} className={`px-3 py-1.5 text-xs font-bold rounded ${activeTab === 'matriculas' ? 'bg-slate-800 text-white' : 'bg-gray-100'}`}>📝 Matricular Aluno</button>
              <button onClick={() => setActiveTab('fichas')} className={`px-3 py-1.5 text-xs font-bold rounded ${activeTab === 'fichas' ? 'bg-slate-800 text-white' : 'bg-gray-100'}`}>📄 Fichas & Certificados</button>
            </div>
              ...
              <div className="space-y-4">
                ...
                <table className="w-full text-xs border text-left">
                  <thead className="bg-gray-100">
                    <tr><th className="p-2 border-r">Nome</th><th className="p-2 border-r">CPF</th><th className="p-2">Email</th></tr>
                  </thead>
                  <tbody>
                    {alunos.map(a => (
            ...
            {activeTab === 'cursos' && (
              <div className="space-y-4">
                ...
                <table className="w-full text-xs border text-left">
                  <thead className="bg-gray-100">
                    <tr><th className="p-2 border-r">Curso</th><th className="p-2 border-r">Carga Horária</th><th className="p-2">Instrutor</th></tr>
                  </thead>
                  <tbody>
                    {cursos.map(c => (
            ...
            {activeTab === 'matriculas' && (
              <form onSubmit={handleCadastrarMatricula} className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs bg-gray-50 p-3 border rounded">
                ...
                <input type="text" placeholder="Nota Final" value={formMatricula.nota} onChange={e => setFormMatricula({...formMatricula, nota: e.target.value})} className="p-2 border rounded" required />
                ...
                <button type="submit" className="bg-slate-800 text-white font-bold py-2 rounded col-span-full">Efetuar Matrícula</button>
              </form>
            )}
            ...
            {activeTab === 'fichas' && (
              <table className="w-full text-xs border text-left">
                <thead className="bg-gray-100">
                  <tr><th className="p-2 border-r">Aluno</th><th className="p-2 border-r">Curso</th><th className="p-2 border-r">Nota</th><th className="p-2 border-r">Situação</th><th className="p-2 text-center">Ação</th></tr>
                </thead>
                <tbody>
                  {matriculas.map(m => (
                    <tr key={m.id} className="border-b">
                      <td className="p-2 border-r font-semibold">{alunos.find(a => a.id === m.alunoId)?.nome}</td>
                      <td className="p-2 border-r">{cursos.find(c => c.id === m.cursoId)?.nome}</td>
